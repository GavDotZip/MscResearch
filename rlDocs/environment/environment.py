...
from .trafficSignals import TrafficSignal
...
class SumoEnvironment(gym.Env):
    ...
    Args:
        ...
        out_csv_name (Optional[str]): name of the .csv output with simulation dqnresults. If None, no output is generated
        ...
        single_agent (bool): If true, it behaves like a regular gym.Env. Else, it behaves like a MultiagentEnv (returns dict of observations, testingResults, dones, infos).
    ...
    def __init__(
            ...
            out_xlsx_name: Optional[str] = None,
            use_gui: bool = False,
            virtual_display: Tuple[int, int] = (3200, 1800),
            begin_time: int = 0,
            num_seconds: int = 18000,
        ...
        self.out_xlsx_name = out_xlsx_name
    ...
    def reset(self, seed: Optional[int] = None, **kwargs):
        ...
        if self.episode != 0:
            ...
            self.save_xlsx(self.out_xlsx_name, self.episode)
        self.episode += 1
        self.metrics = []
    ...
    def _get_system_info(self):
        ...
        people = [self.sumo.vehicle.getPersonCapacity(vehicle) for vehicle in vehicles]
        emissions = [self.sumo.vehicle.getCOEmission(vehicle) for vehicle in vehicles]
        ...
        air = [traci.vehicle.getPMxEmission(vehicle) for vehicle in vehicles]
        return {
            # In SUMO, a vehicle is considered halting if its speed is below 0.1 m/s
            "system_total_passengers": sum(people),
            "system_total_stopped": sum(int(speed < 0.1) for speed in speeds),
            "Waiting_Times": sum(waiting_times),
            "Total_Air_Pollution": sum(emissions),
            "Fuel_Consumption": sum(fuel),
            "system_total_air_pollution": sum(air),
    ...
    def save_xlsx(self, out_xlsx_name, episode):
        if out_xlsx_name is not None:
            dataset = pd.DataFrame(self.metrics)
            Path(Path(out_xlsx_name).parent).mkdir(parents=True, exist_ok=True)
            dataset.to_excel(out_xlsx_name + f"Stats.xlsx", index=False)
...
class SumoEnvironmentPZ(AECEnv, EzPickle):
    ...
    def __init__(self, my_reward_fn=None, **kwargs):
        """Initialize the environment."""
        EzPickle.__init__(self, **kwargs)
        self._kwargs = kwargs

        self.seed()
        self.env = SumoEnvironment(**self._kwargs, reward_fn=my_reward_fn)
    ...
    def save_xlsx(self, out_xlsx_name, episode):
        """Save metrics of the simulation to a .xlsx file."""
        self.env.save_xlsx(out_xlsx_name, episode)
