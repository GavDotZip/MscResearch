...
class SumoEnvironment(gym.Env):
    ...
    def reset(self):
        if self.run != 0:
            ...
            self.save_xlsx(self.out_xlsx_name, self.run)
    ...
    def save_csv(self, out_xlsx_name, run):
        if out_xlsx_name is not None:
            df = pd.DataFrame(self.metrics)
            Path(Path(out_xlsx_name).parent).mkdir(parents=True, exist_ok=True)
            df.to_csv(out_xlsx_name + '_conn{}_run{}'.format(self.label, run) + '.csv', index=False)
...
class SumoEnvironmentPZ(AECEnv, EzPickle):
    ...
    def save_csv(self, out_xlsx_name, run):
        self.env.save_csv(out_xlsx_name, run)
